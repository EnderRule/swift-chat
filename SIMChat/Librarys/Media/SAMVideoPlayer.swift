...
private extension SAMVideoPlayer {
    ...
    @discardableResult func _active() -> Bool {
        ...
        _isActived = true
        do {
            try SAMAudioSession.setCategory(AVAudioSessionCategoryPlayback)
            try SAMAudioSession.setActive(true, context: self)
            return true
        } catch {
            return false
        }
    ...
    @discardableResult func _deactive() -> Bool {
        ...
        _isActived = false
        do {
            try SAMAudioSession.setActive(false, with: .notifyOthersOnDeactivation, context: self)
            return true
        } catch {
            return false
        }
