...
/// 音频录音器
///
open class SMAudioRecorder: NSObject, AVAudioRecorderDelegate {
    ...
    fileprivate func performForError(_ code: Int, _ error: String) {
        let error = NSError(domain: "SMAudioDoMain", code: code, userInfo: [NSLocalizedFailureReasonErrorKey: error])
    ...
    open var settings: [String : Any] {
     ...
     // the delegate will be sent messages from the SMAudioRecorderDelegate protocol
    open weak var delegate: SMAudioRecorderDelegate?
...
/// 音频播放器代理
///
@objc public protocol SMAudioRecorderDelegate {

    @objc optional func audioRecorder(shouldPreparing audioRecorder: SMAudioRecorder) -> Bool
    @objc optional func audioRecorder(didPreparing audioRecorder: SMAudioRecorder)
    
    @objc optional func audioRecorder(shouldRecording audioRecorder: SMAudioRecorder) -> Bool
    @objc optional func audioRecorder(didRecording audioRecorder: SMAudioRecorder)
    
    @objc optional func audioRecorder(didPause audioRecorder: SMAudioRecorder)
    
    @objc optional func audioRecorder(didStop audioRecorder: SMAudioRecorder)
    @objc optional func audioRecorder(didInterruption audioRecorder: SMAudioRecorder)
    
    // audioRecorderDidFinishRecording:successfully: is called when a recording has been finished or stopped. This method is NOT called if the recorder is stopped due to an interruption. 
    @objc optional func audioRecorder(didFinishRecording audioRecorder: SMAudioRecorder, successfully flag: Bool)
    
    // if an error occurs will be reported to the delegate.
    @objc optional func audioRecorder(didOccur audioRecorder: SMAudioRecorder, error: Error?)
}
