...
internal class SAInputAccessoryViewLayout: UICollectionViewLayout {
    ...
    override func initialLayoutAttributesForAppearingItem(at itemIndexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if reload.contains(itemIndexPath) {
            ...
            _logger.trace("is reload at \((itemIndexPath as NSIndexPath).item)-\((itemIndexPath as NSIndexPath).section) => \(attr?.frame)")
    ...
    private func _layoutAttributesForItemAtIndexPath(_ indexPath: IndexPath) -> Attributes? {
        ...
        _logger.debug("not found \((indexPath as NSIndexPath).item) - \((indexPath as NSIndexPath).section)")
        return nil
    }
