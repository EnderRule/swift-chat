...
internal enum SPPickerPreviewDirection {
    case normal
    case reverse
}

internal class SPPreviewer: UIViewController {
    
    var direction: SPPickerPreviewDirection = .normal
    ...
    weak var picker: SPPickerInternal?
    weak var selection: SPSelectionable?
    
    var previewingItem: AnyObject?
    weak var previewingDelegate: SPPreviewableDelegate?
    ...
    func scroll(to photo: SPAsset?, animated: Bool) {
...
//        _toolbar.items = toolbarItems
        ...
        _contentView.register(SPPreviewerCell.self, forCellWithReuseIdentifier: "Unknow")
        _contentView.register(SPPreviewerImageCell.self, forCellWithReuseIdentifier: "Image")
        _contentView.register(SPPreviewerVideoCell.self, forCellWithReuseIdentifier: "Video")
...
//            _updateToolbar(_isFullscreen, animated: false)
    ...
    init(picker: SPPickerInternal, options: SPPickerOptions) {
    ...
    fileprivate lazy var _contentViewLayout: SPPreviewerLayout = SPPreviewerLayout()
...
extension SPPreviewer: SPPreviewableDelegate {
    
    func fromPreviewable(with item: AnyObject) -> SPPreviewable? {
        _logger.trace()
        
        guard let photo = item as? SPAsset else {
            return nil
        }
        ...
        return cell as? SPPreviewerCell
    }
    ...
    func toPreviewable(with item: AnyObject) -> SPPreviewable? {
        ...
        // 第一次肯定是没有加载的, 所以生成一个SPBrowserViewFastPreviewing
        return SPBrowserViewFastPreviewing(photo: photo, view: view)
    }
    
    func previewable(_ previewable: SPPreviewable, willShowItem item: AnyObject) {
        _logger.trace()
        view.isHidden = true
    }
    func previewable(_ previewable: SPPreviewable, didShowItem item: AnyObject) {
        _logger.trace()
        view.isHidden = false
    }
}
...
// MARK: - SPBrowserViewDelegate

extension SPPreviewer: SPPreviewerCellDelegate {
    
    func previewerCell(_ previewerCell: SPPreviewerCell, didTap photo: SPAsset) {
        _logger.trace()
        
        _updateIsFullscreen(!_isFullscreen, animated: true)
    }
    func previewerCell(_ previewerCell: SPPreviewerCell, didDoubleTap photo: SPAsset) {
        _logger.trace()
    ...
    func previewerCell(_ previewerCell: SPPreviewerCell, shouldRotation photo: SPAsset) -> Bool {
        _logger.trace()
    ...
    func previewerCell(_ previewerCell: SPPreviewerCell, didRotation photo: SPAsset, orientation: UIImageOrientation) {
        _logger.trace()
...
extension SPPreviewer: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    public func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        guard let cell = cell as? SPPreviewerCell else {
            return
        }
...
extension SPPreviewer: PHPhotoLibraryChangeObserver {
    ...
    public func photoLibraryDidChange(_ changeInstance: PHChange) {
        ...
        guard let result = _photosResult, let change = changeInstance.changeDetails(for: result), change.hasIncrementalChanges else {
            ...
            if let album = _album, !SPAlbum.albums.contains(album) {
