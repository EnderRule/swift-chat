...
internal class SPPickerAlbumsStackView: UIView, SPProgressiveableObserver {
    ...
    var album: SPAlbum? {
        didSet {
            ...
            _updateIcon(SPBadge(collectionSubtype: newValue.subtype))
            _updatePhotos(newValue.photos(with: newResult, in: range).reversed())
        }
    }
    
    func progressiveable(_ progressiveable: SPProgressiveable, didChangeContent content: Any?) {
    ...
    private func _updateIcon(_ badge: SPBadge) {
    ...
    private func _updatePhotos(_ photos: [SPAsset]) {
        ...
        _imageLayers.enumerated().forEach { 
            ...
            _setImage(photo.image(with: size) as? SPProgressiveableImage, at: $0.offset)
        }
    ...
    private func _setImage(_ newValue: SPProgressiveableImage?, at index: Int) {
    ...
    private var _badgeView: SPBadgeView?
    
    private lazy var _images: [Int: SPProgressiveableImage?] = [:]
