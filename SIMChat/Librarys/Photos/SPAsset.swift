...
public class SPAsset: NSObject {
    ...
    public var image: UIImage? {
        return image(with: SPhotoMaximumSize)
    }
    ...
    public func image(with size: CGSize) -> UIImage? {
        return SPLibrary.shared.image(with: self, size: size)
    }
    ...
    public func data(with handler: @escaping (Data?) -> Void)  {
        ...
        return SPLibrary.shared.data(with: self) { [weak self](data, dataUTI, orientation, info) in
            self?.data = data
            handler(data)
        }
    }
    ...
    public func playerItem(with handler: @escaping (AVPlayerItem?) -> Void) {
        ...
        SPLibrary.shared.playerItem(with: self) { [weak self](item, info) in
    ...
    public override func isEqual(_ object: Any?) -> Bool {
        guard let photo = object as? SPAsset else {
            return false
        }
    ...
    public init(asset: PHAsset, album: SPAlbum) {
        self.asset = asset
        self.album = album
        super.init()
    }
...
internal func SPStringForDuration(_ duration: TimeInterval) -> String {
    let s = Int(duration) % 60
    let m = Int(duration / 60)
    return String(format: "%02zd:%02zd", m, s)
}

internal func SPStringForBytesLenght(_ len: Int) -> String {
    if len <= 999 {
        // 只显示1B-999B
