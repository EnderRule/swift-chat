...
@objc public protocol SPContainterViewDelegate {
    
    @objc optional func containterViewDidScroll(_ containterView: SPContainterView) // any offset changes
    @objc optional func containterViewDidZoom(_ containterView: SPContainterView) // any zoom scale changes
    @objc optional func containterViewDidRotation(_ containterView: SPContainterView) // any rotation changes
    
    @objc optional func viewForZooming(in containterView: SPContainterView) -> UIView? // return a view that will be scaled. if delegate returns nil, nothing happens
    
    // called on start of dragging (may require some time and or distance to move)
    @objc optional func containterViewWillBeginDragging(_ containterView: SPContainterView)

    // called on finger up if the user dragged. velocity is in points/millisecond. targetContentOffset may be changed to adjust where the scroll view comes to rest
    @objc optional func containterViewWillEndDragging(_ containterView: SPContainterView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>)

    // called on finger up if the user dragged. decelerate is true if it will continue moving afterwards
    @objc optional func containterViewDidEndDragging(_ containterView: SPContainterView, willDecelerate decelerate: Bool)

    @objc optional func containterViewWillBeginDecelerating(_ containterView: SPContainterView) // called on finger up as we are moving
    @objc optional func containterViewDidEndDecelerating(_ containterView: SPContainterView) // called when scroll view grinds to a halt

    @objc optional func containterViewDidEndScrollingAnimation(_ containterView: SPContainterView) // called when setContentOffset/scrollRectVisible:animated: finishes. not called if not animating

    @objc optional func containterViewShouldScrollToTop(_ containterView: SPContainterView) -> Bool // return a yes if you want to scroll to the top. if not defined, assumes YES
    @objc optional func containterViewDidScrollToTop(_ containterView: SPContainterView) // called when scrolling animation finished. may be called immediately if already at top
    
    @objc optional func containterViewWillBeginZooming(_ containterView: SPContainterView, with view: UIView?) // called before the scroll view begins zooming its content
    @objc optional func containterViewDidEndZooming(_ containterView: SPContainterView, with view: UIView?, atScale scale: CGFloat) // scale between minimum and maximum. called after any 'bounce' animations

    @objc optional func containterViewShouldBeginRotationing(_ containterView: SPContainterView, with view: UIView?) -> Bool // called before the scroll view begins zooming its content
    @objc optional func containterViewDidEndRotationing(_ containterView: SPContainterView, with view: UIView?, atOrientation orientation: UIImageOrientation) // scale between minimum and maximum. called after any 'bounce' animations
}


@objc public class SPContainterView: UIView {
    
    public weak var delegate: SPContainterViewDelegate?
...
fileprivate extension SPContainterView {
...
extension SPContainterView: UIGestureRecognizerDelegate, UIScrollViewDelegate {
