...
public class SPAlbum: NSObject {
    ...
    public override func isEqual(_ object: Any?) -> Bool {
        guard let collection = object as? SPAlbum else {
            return false
        }
    ...
    public func photos(with result: PHFetchResult<PHAsset>) -> [SPAsset] {
        var photos: [SPAsset] = []
        result.enumerateObjects({
            let photo = SPAsset(asset: $0.0, album: self)
            photos.append(photo)
        })
        return photos
    }
    public func photos(with result: PHFetchResult<PHAsset>, in range: NSRange) -> [SPAsset] {
    ...
    /// 获取所有相册
    public static var albums: [SPAlbum] {
        return _fetchAssetCollections()
    }
    /// 获取moment相册
    public static var momentAlbum: SPAlbum? {
        return _fetchAssetCollections(with: .moment, subtype: .any).first
    }
    /// 获取历史相册
    public static var recentlyAlbum: SPAlbum? {
...
extension SPAlbum {
    ...
    fileprivate static func _fetchAssetCollections() -> [SPAlbum] {
    ...
    fileprivate static func _fetchAssetCollections(with type: PHAssetCollectionType, subtype: PHAssetCollectionSubtype, options: PHFetchOptions? = nil) -> [SPAlbum] {
        var albums: [SPAlbum] = []
        PHAssetCollection.fetchAssetCollections(with: type, subtype: subtype, options: nil).enumerateObjects({ 
            albums.append(SPAlbum(collection: $0.0))
        })
        return albums
    }
}
