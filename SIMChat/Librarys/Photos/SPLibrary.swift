...
public class SPLibrary: NSObject {
   
    
    public func isExists(of photo: SPAsset) -> Bool {
    ...
    public func image(with photo: SPAsset, size: CGSize) -> UIImage? {
        //_logger.trace()
        ...
        _allCaches[photo.identifier]?[name] = SPWKObject(object: image)
        _queue.async {
    ...
    public func imageForAlmost(with photo: SPAsset, size: CGSize) -> UIImage? {
        //_logger.trace()
        ...
        // 查找
        caches.forEach {
            guard let img = $1.object as? SPProgressiveableImage else {
                return
            }
            let os = image?.size ?? .zero
            let ns = img.size
            // 必须小于或者等于size
            guard (ns.width <= size.width && ns.height <= size.height) || (size == SPhotoMaximumSize) else {
                return
            }
            // 必须大于当前的图片
    ...
    public func data(with photo: SPAsset, resultHandler: @escaping (Data?, String?, UIImageOrientation, [AnyHashable : Any]?) -> Void) {
        //_logger.trace(photo.identifier)
        
        _requestImageData(photo, nil, resultHandler: resultHandler)
    }
    
    public func playerItem(with photo: SPAsset, resultHandler: @escaping (AVPlayerItem?, [AnyHashable : Any]?) -> Void) {
...
//    public func requestImage(for photo: SPAsset, targetSize: CGSize, contentMode: PHImageContentMode = .default, options: PHImageRequestOptions? = nil, resultHandler: @escaping (UIImage?, [AnyHashable : Any]?) -> Void) -> PHImageRequestID {
...
//    }
//    public static func requestImageData(for photo: SPAsset, options: PHImageRequestOptions? = nil, resultHandler: @escaping (Data?, String?, UIImageOrientation, [AnyHashable : Any]?) -> Swift.Void) {
//        let im = PHCachingImageManager.default()
//        im.requestImageData(for: photo.asset, options: options, resultHandler: resultHandler)
//    }
    
    private func _requestImage(_ photo: SPAsset, _ size: CGSize, _ contentMode: PHImageContentMode, _ options: PHImageRequestOptions?, resultHandler: @escaping (UIImage?, [AnyHashable : Any]?) -> Void) {
        let im = PHCachingImageManager.default()
        im.requestImage(for: photo.asset, targetSize: size, contentMode: contentMode, options: options, resultHandler: resultHandler)
    }
    
    private func _requestPlayerItem(_ photo: SPAsset, _ options: PHVideoRequestOptions?, resultHandler: @escaping (AVPlayerItem?, [AnyHashable : Any]?) -> Void) {
        let im = PHCachingImageManager.default()
        im.requestPlayerItem(forVideo: photo.asset, options: options, resultHandler: resultHandler)
    }
    
    private func _requestImageData(_ photo: SPAsset, _ options: PHImageRequestOptions?, resultHandler: @escaping (Data?, String?, UIImageOrientation, [AnyHashable : Any]?) -> Void) {
        let im = PHCachingImageManager.default()
        im.requestImageData(for: photo.asset, options: options, resultHandler: resultHandler)
    }
...
//        // The options values shall exactly match the options values used in loading methods. If two or more caching requests are done on the SMe asset using different options or different targetSize the first
//        // caching request will have precedence (until it is stopped)
    public func startCachingImages(for assets: [SPAsset], targetSize: CGSize, contentMode: PHImageContentMode, options: PHImageRequestOptions?) {
    ...
    public func stopCachingImages(for assets: [SPAsset], targetSize: CGSize, contentMode: PHImageContentMode, options: PHImageRequestOptions?) {
        let im = PHCachingImageManager.default() as! PHCachingImageManager
        let ass = assets.map {
            return $0.asset
        }
    ...
    public static var shared: SPLibrary = {
        let lib = SPLibrary()
    ...
    private lazy var _queue: DispatchQueue = DispatchQueue(label: "SPhotoImageLoadQueue")
    private lazy var _allCaches: [String: [String: SPWKObject<UIImage>]] = [:]
}

extension SPLibrary: PHPhotoLibraryChangeObserver {
...
private func _SPhotoResouceId(_ photo: SPAsset, size: CGSize) -> UInt {
    guard size != SPhotoMaximumSize else {
        return UInt.max
    }
    return UInt(size.width) / 16
}
private func _SPhotoResouceSize(_ photo: SPAsset, size: CGSize) -> CGSize {
    let id = _SPhotoResouceId(photo, size: size)
    guard id != .max else {
        return SPhotoMaximumSize
    }
...
public let SPhotoMaximumSize = PHImageManagerMaximumSize
