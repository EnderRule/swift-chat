...
internal class SPAssetView: UIImageView, SPPreviewable {
    ...
    var photo: SPAsset? {
        didSet {
            ...
            _updateIcon(SPBadge(photo: newValue))
    ...
    weak var delegate: SPSelectionable?
    ...
    private func _updateIcon(_ badge: SPBadge) {
        ...
        if _badgeView?.superview == nil {
            let view = SPBadgeView(style: .small)
    ...
    private func _updateSelection(with photo: SPAsset?, animated: Bool) {
