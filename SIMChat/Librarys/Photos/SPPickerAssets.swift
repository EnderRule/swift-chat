...
internal class SPPickerAssets: UICollectionViewController, UIGestureRecognizerDelegate {
    ...
    override func viewDidLoad() {
        ...
        collectionView?.register(SPPickerAssetsCell.self, forCellWithReuseIdentifier: "Item")
        collectionView?.register(SPPickerAssetsFooter.self, forSupplementaryViewOfKind: UICollectionElementKindSectionFooter, withReuseIdentifier: "Footer")
    ...
    @objc private func panHandler(_ sender: UIPanGestureRecognizer) {
        ...
        let operatorType = _batchIsSelectOperator ?? {
            ...
            guard let cell = collectionView?.cellForItem(at: IndexPath(item: nidx, section: 0)) as? SPPickerAssetsCell else {
                return false
            }
    ...
    private func _cachePhotos(_ photos: [SPAsset]) {
//        // 缓存加速
...
//        SPLibrary.shared.startCachingImages(for: photos, targetSize: size, contentMode: .aspectFill, options: options)
    ...
    fileprivate func _updateStatus(_ newValue: SPhotoStatus) {
        ...
        case .notData:
            let error = _statusView ?? SPErrorView()
    ...
    fileprivate func _updateSelection(_ newValue: Bool, at index: Int) -> Bool {
        ...
        if let cell = collectionView?.cellForItem(at: idx) as? SPPickerAssetsCell {
    ...
    init(picker: SPPickerInternal, album: SPAlbum) {
        super.init(collectionViewLayout: {
            let layout = SPPickerAssetsLayout()
    ...
    private var _status: SPhotoStatus = .notError
    private var _statusView: SPErrorView?
    
    private var _toolbarItems: [UIBarButtonItem]??
    
    fileprivate weak var _footer: SPPickerAssetsFooter?
...
extension SPPickerAssets {
    ...
    func didSelectItem(_ sender: Notification) {
        ...
        collectionView?.visibleCells.forEach {
            let cell = $0 as? SPPickerAssetsCell
    ...
    func didDeselectItem(_ sender: Notification) {
        guard let _ = sender.object as? SPAsset else {
            return
        }
        logger.trace()
        collectionView?.visibleCells.forEach {
...
extension SPPickerAssets: UICollectionViewDelegateFlowLayout {
    ...
    override func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if let footer = view as? SPPickerAssetsFooter {
            _footer = footer
            _updateFooter()
        }
        return view
    ...
    override func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        guard let cell = cell as? SPPickerAssetsCell else {
            return
        }
...
extension SPPickerAssets: SPPreviewableDelegate {
    ...
    func fromPreviewable(with item: AnyObject) -> SPPreviewable? {
        ...
        return (cell as? SPPickerAssetsCell)?.photoView
    }
...
extension SPPickerAssets: SPSelectionable {
...
extension SPPickerAssets: PHPhotoLibraryChangeObserver {
