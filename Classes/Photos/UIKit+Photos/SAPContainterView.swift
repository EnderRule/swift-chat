...
@objc public protocol SAPContainterViewDelegate {
    
    @objc optional func containterViewDidScroll(_ containterView: SAPContainterView) // any offset changes
    @objc optional func containterViewDidZoom(_ containterView: SAPContainterView) // any zoom scale changes
    @objc optional func containterViewDidRotation(_ containterView: SAPContainterView) // any rotation changes
    
    @objc optional func viewForZooming(in containterView: SAPContainterView) -> UIView? // return a view that will be scaled. if delegate returns nil, nothing happens
    
    // called on start of dragging (may require some time and or distance to move)
    @objc optional func containterViewWillBeginDragging(_ containterView: SAPContainterView)

    // called on finger up if the user dragged. velocity is in points/millisecond. targetContentOffset may be changed to adjust where the scroll view comes to rest
    @objc optional func containterViewWillEndDragging(_ containterView: SAPContainterView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>)

    // called on finger up if the user dragged. decelerate is true if it will continue moving afterwards
    @objc optional func containterViewDidEndDragging(_ containterView: SAPContainterView, willDecelerate decelerate: Bool)

    @objc optional func containterViewWillBeginDecelerating(_ containterView: SAPContainterView) // called on finger up as we are moving
    @objc optional func containterViewDidEndDecelerating(_ containterView: SAPContainterView) // called when scroll view grinds to a halt

    @objc optional func containterViewDidEndScrollingAnimation(_ containterView: SAPContainterView) // called when setContentOffset/scrollRectVisible:animated: finishes. not called if not animating

    @objc optional func containterViewShouldScrollToTop(_ containterView: SAPContainterView) -> Bool // return a yes if you want to scroll to the top. if not defined, assumes YES
    @objc optional func containterViewDidScrollToTop(_ containterView: SAPContainterView) // called when scrolling animation finished. may be called immediately if already at top
    
    @objc optional func containterViewWillBeginZooming(_ containterView: SAPContainterView, with view: UIView?) // called before the scroll view begins zooming its content
...
@objc public class SAPContainterView: UIView {
    
    public weak var delegate: SAPContainterViewDelegate?
...
fileprivate extension SAPContainterView {
...
extension SAPContainterView: UIGestureRecognizerDelegate, UIScrollViewDelegate {
